...
const initialRoles = [
  {
    id: 1,
    nome: 'Admin',
    descricao: 'Acesso total à plataforma e gerenciamento da equipe.',
    permissoes: ['Módulos', 'Financeiro', 'Equipe', 'Relatórios'],
  ...
  },
  {
    id: 2,
    nome: 'TI',
    descricao: 'Responsável por processos técnicos e suporte.',
  ...
  },
  {
    id: 3,
    nome: 'RH',
    descricao: 'Gestão de pessoas, admissões e recrutamento.',
    permissoes: ['Módulos', 'Equipe', 'Relatórios'],
  ...
  },
  {
    id: 4,
    nome: 'Oper.',
    descricao: 'Operação do dia a dia e acompanhamento de tarefas.',
    permissoes: ['Módulos'],
  ...
  },
  {
    id: 5,
    nome: 'Vendas',
    descricao: 'Acompanha clientes, pipeline e indicadores.',
    permissoes: ['Módulos', 'Financeiro', 'Relatórios'],
  ...
  },
  {
    id: 6,
    nome: 'Estág.',
    descricao: 'Acesso mais restrito para aprendizado e apoio.',
    permissoes: ['Relatórios'],
...
const initialMembers = [
  { id: 1, nome: 'Camila Morais', email: 'camila@levit.app', cargo: 'Admin', status: 'Ativo' },
  { id: 2, nome: 'Geraldo Bonito', email: 'geraldobonito@levit.app', cargo: 'TI', status: 'Ativo' },
  { id: 3, nome: 'Ana Krebs', email: 'anakrebs@levit.app', cargo: 'RH', status: 'Ativo' },
  { id: 4, nome: 'Ricardo F.', email: 'ricardofonseca@levit.app', cargo: 'Oper.', status: 'Ativo' },
  { id: 5, nome: 'Maria Luiza', email: 'marialuiza@levit.app', cargo: 'Vendas', status: 'Ativo' },
  { id: 6, nome: 'Paulo Cezar', email: 'paulocezar@levit.app', cargo: 'TI', status: 'Ativo' },
  { id: 7, nome: 'Oldair Antonio Sebastiao', email: 'oldairantoniosebastiao@levit.app', cargo: 'Estág.', status: 'Ativo' },
];
...
export default function TeamManagement() {
  ...
  const handleSaveRole = (e) => {
    ...
    const usuarios = members.filter(
      (member) => member.cargo === (editingRole?.nome || roleForm.nome)
    ).length;
    ...
    if (editingRole) {
      setRoles((current) =>
        current.map((role) =>
          role.id === editingRole.id
            ? { ...role, ...roleForm, usuarios }
            : role
        )
      );

      if (editingRole.nome !== roleForm.nome) {
        setMembers((current) =>
          current.map((member) =>
            member.cargo === editingRole.nome
              ? { ...member, cargo: roleForm.nome }
              : member
          )
        );
      }
      ...
      setRoles((current) => [
        ...current,
        { id: Date.now(), ...roleForm, usuarios: 0 },
      ]);
    }
  ...
  const handleDeleteRole = (roleId) => {
    ...
    const isUsed = members.some((member) => member.cargo === role.nome);

    if (isUsed) {
      window.alert(
        'Esse cargo está vinculado a colaboradores. Edite ou troque os usuários de cargo antes de excluir.'
      );
      return;
    }

    if (!window.confirm(`Deseja excluir o cargo "${role.nome}"?`)) return;
            ...
            >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setMemberForm((current) => ({ ...current, cargo: e.target.value }))
                  }
                  className="w-full border border-divider rounded-lg px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                >
                  {roles.map((role) => (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setMemberForm((current) => ({ ...current, status: e.target.value }))
                  }
                  className="w-full border border-divider rounded-lg px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                >
